...
inline char ucChar2Int(char ch){
...
inline char ucStr2Bin(char *str){
        char tempWord[2];
        char chn;

        tempWord[0] = ucChar2Int(str[0]);                                //make the B to 11 -- 00001011
        tempWord[1] = ucChar2Int(str[1]);                                //make the 0 to 0  -- 00000000
...
inline std::string urldecode(const std::string& str){
    ...
    inline unsigned char uctoHex(const unsigned char &x)
    {
    ...
    // cout << "size: " << sIn.size() << endl;
        ...
        {       
            ...
            {
                buf[0] = '%';
                buf[1] = uctoHex( (unsigned char)sIn[ix] >> 4 );
                buf[2] = uctoHex( (unsigned char)sIn[ix] % 16);
            }
            sOut += (char *)buf;
        }
        return sOut;
...
inline std::string Url2Str_gb2312(std::string str){
