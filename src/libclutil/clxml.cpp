...
#define XML_HEAD "<?xml version=\"1.0\" encoding=\"gbk\" standalone=\"yes\"?>"
	...
	int xmlAtrri::load_string(const char* szAttri)
	{
		clear();
		if (NULL == szAttri)
			return 0;
	...
	const char* xmlAtrri::get_attri(const char* name)
	{
		ANode_t *ptr;
		assert(name);
		ptr = head.next;
	...
	int xmlAtrri::set_attri(const char* name, const char* val)
	{
	...
	bool xmlAtrri::check_attri(const char* name, const char* val)
	{
	...
	int xmlAtrri::to_string(char* buf, int buflen)
	{
		if (buflen<size())
			return -1;
		ANode_t *ptr = head.next;
	...
	bool clxml::string_english_only(const char* str)
	{
	...
	int clxml::load_file(const char* path)
	{
		clear();
		char *szXML = NULL;
		int size = 0;
	...
	int clxml::load_string(const char* szXML)
	{
	...
	int clxml::set_xml_head(const char* head)
	{
		if (!head)
			return -1;
	...
	xmlNode *clxml::find_first_node(const char* tagPath, xmlNode* start/*=NULL*/)
	{
	...
	xmlNode *clxml::find_first_node_attri(const char* tagPath, const char* attri_name, const char* attri_val, xmlNode* start/*=NULL*/)
	{
		xmlNode *n = find_first_node(tagPath, start);
	...
	char* clxml::find_first_node_data(xmlNode* start, const char* tagPath, const char* val_default, char buf[], int bufsize)
	{
		//不管有无节点,只要空值就使用默认
		xmlNode *n = find_first_node(tagPath, start);
		const char* d = NULL;
	...
	const char* clxml::get_node_data(const char* tagPath, xmlNode* start/*=NULL*/)
	{
	...
	xmlNode *clxml::find_node(const char* tag, int tagLen, const char* childTag, xmlNode *node)
	{
		if (0 == tagLen)
			return NULL;
		xmlNode *p = NULL;
	...
	int clxml::delete_node(xmlNode *node)
	{
		if (NULL == node)
			return -1;
		if (node->pparent)
	...
	//**************************
	int clxml::load_next(const char* szMin, const char* szMax, xmlNode* head)
	{
	...
	int clxml::load_child(const char* szMin, const char* szMax, xmlNode* head)
	{
	...
	const char* clxml::find_tag_end(const char* tag, const char* szMin, const char* szMax)
	{
		const char *p1=NULL, *p2, *p3;
	...
	int clxml::save_file(const char* path)
	{
		int isize = size();
		if (!isize)
			return -1;
	...
	int clxml::size_count(xmlNode* head, int step)
	{
		if (!head)
			return 0;
		int size = 0;
	...
	int clxml::to_string(char* szXML, int/* maxSize*/)
	{
		if (!m_head.pnext)
			return -1;
		szXML[0] = '\0';
	...
	int clxml::format_string(char* szXML, xmlNode* head, int step)
	{
		if (!head)
			return 0;
		strcat(szXML, "\r\n");
	...
	xmlNode *clxml::add_node(const char* path, const char* data/*=NULL*/)
	{
	...
	xmlNode *clxml::add_child(xmlNode *node, const char* tag, const char* data/*=NULL*/)
	{
	...
	void clxml::delete_node_root(xmlNode* head, bool delnext)
	{
		if (head)
		{
